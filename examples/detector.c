...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    printf("Start trainning a new net >> Learning Rate: %g, Momentum: %g, Decay: %g\n", net.learning_rate, net.momentum, net.decay);
    data train, buffer;
    ...
    while(get_current_batch(net) < net.max_batches){
        if(l.random && count++%10 == 0){
            printf("Resizing\n");
            int dim = (rand() % 10 + 10) * 32;//radomly change the input dimensions to improve the generalization of the net
            if (get_current_batch(net)+200 > net.max_batches) dim = 640;
        ...
        */
        ...
        if (!isnan(loss))
            ...
            avg_loss = avg_loss;
            printf(">> current loss = NAN, skip this loss value, use the previous avg_loss instead\n");
        i = get_current_batch(net);
        printf("batch No.%ld: Loss %f, avg_loss %f,current_lr %f, %lf secs, %d images\n", get_current_batch(net), loss, avg_loss, get_current_rate(net), sec(clock()-time), i*imgs);
        if(i%1000==0){
#ifdef GPU
            ...
            sprintf(buff, "%s/%s.backup", backup_directory, base);
						printf("current_batch = n*1k,save a backup\n");
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        //draw_detections_eval(im, l.w*l.h*l.n, thresh, boxes, probs, names, alphabet, l.classes, filename);
